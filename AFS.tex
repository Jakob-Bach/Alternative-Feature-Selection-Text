...
To account for varying dataset size, we put the ratio between feature-set size~$k$ and dataset-size~$n$ on the x-axis.
...
In particular, over all experimental settings, decision trees and random forests both have a mean training-set MCC of 0.86 and a median training-set MCC of 1.0.
In contrast, on the test set, decision trees have a mean training-set MCC of 0.48 (median: 0.54), while random forests have a slightly higher mean training-set MCC of 0.53 (median: 0.63).
Thus, test-set prediction performance is significantly worse on average.
For more details, Figure~\ref{fig:afs:evaluation-metrics-overfitting} shows the distribution of difference between training-set quality and test-set quality over the different experimental settings.
Again, we observe that training-set quality usually is higher, though there are a few cases where the difference becomes negative, i.e., test-set quality is higher.
...
\begin{figure}[htb]
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{Max-normalization. Objective values of infeasible feature sets excluded.}
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{Max-normalization. Objective values of infeasible feature sets set to zero.}
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{Min-max-normalization. Objective values of infeasible feature sets excluded.}
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{Min-max-normalization. Objective values of infeasible feature sets set to zero.}
		\label{fig:afs:impact-num-alternatives-objective-min-max-fillna}
	\end{subfigure}

	\caption{
		Objective value $Q$, normalized per experimental setting, over the number of alternatives.
...
Figures~\ref{fig:afs:impact-num-alternatives-objective-max-fillna} and~\ref{fig:afs:impact-num-alternatives-objective-min-max-fillna} show the same data as Figures~\ref{fig:afs:impact-num-alternatives-objective-max} and~\ref{fig:afs:impact-num-alternatives-objective-min-max} but with the objective of infeasible feature sets set to zero.
This makes sense since selecting no features leads to such an objective value for \emph{MI}-based feature selection.
