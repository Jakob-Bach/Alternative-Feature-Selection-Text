...
Each selected feature' correlation with the prediction target must exceed a pre-defined threshold and the correlation of each other selected feature with the given one.
...
In particular, one only needs to conduct such computations once and can then re-use them in each search for alternatives.
...
Further, even if we only evaluate our own approaches, we can still put the feature-set quality of the alternatives into perspective by comparing them to each other.
