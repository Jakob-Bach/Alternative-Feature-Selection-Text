...
Thus, when we analyzing the search for alternatives in the following, we will normalize feature-set quality appropriately, to compensate for the differences between datasets.
...
Also, overfitting occurs for both models, i.e., there is a gap between training and test performance.
As we do not limit the growth of trees or prune them after training, this observation is expected.
However, that will not prevent our analysis how prediction quality develops over alternative feature sets, where we normalize prediction quality.
