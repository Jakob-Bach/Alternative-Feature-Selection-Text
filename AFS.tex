...
E.g., the univariate objective (cf.~Equation~\ref{eq:afs:univariate-filter}) has an effort in $O(n)$ or even $O(k)$ since it only sums up the individual features' qualities.
...
\end{proposition}
%
Thus, the runtime remains polynomial under the given assumptions and belongs to the parameterized complexity class~$\mathcal{XP}$ in general, considering~$k$ as parameter.
...
\label{sec:afs:appendix:complexity:np-problems}
...
However, since alternative feature selection only uses special types of constraints instead of expressing arbitrary integer linear problems, it could still be easier than $\mathcal{NP}$.

\paragraph{Further assumptions for this section}
...
Note that the latter setting explicitly violates the assumptions of the previous section (cf.~Appendix~\ref{sec:afs:appendix:complexity:exhaustive}).
Thus, subsequent $\mathcal{NP}$-hardness results for the special case of alternative feature selection analyzed here do not contradict prior results of polynomial runtime for other special cases.

Besides~$\tau = 1$, we assume univariate feature qualities as in Equation~\ref{eq:afs:univariate-filter} and Appendix~\ref{sec:afs:appendix:complete-optimization-problem}, i.e., a feature set's quality equals the sum of its constituent features' qualities.
This simplicity of this objective does not only make its evaluation cheap but also eases a transformation from and to well-known $\mathcal{NP}$-hard problems with comparably straightforward objectives.

\paragraph{Number partitioning}

There are several $\mathcal{NP}$-complete problems that involve partitioning a set of elements into non-overlapping subsets~\cite{garey2003computers}, e.g., \textsc{Partition}~\cite{karp1972reducibility} and \textsc{3-Partition}~\cite{garey2003computers}.
...
A key factor driving the hardness of partitioning problems is the shear number of possible partitionings.
...
\paragraph{Number partitioning -- Summed-quality objective}
...
\paragraph{Number partitioning -- Minimum-quality objective with unconstrained feature-set size}
...
Further, besides approximation algorithms, the problem also has polynomial-time exact algorithms assuming certain parameters of the problem to be fixed, e.g., the number of distinct numbers to be partitioned, i.e., feature qualities, or the maximum number, i.e., feature quality~\cite{mnich2018parameterized}.
Thus, the problem is fixed-parameter tractable ($\mathcal{FPT}$) for an appropriate definition of `parameter'.

\paragraph{Number partitioning -- Minimum-quality objective with constrained feature-set size}
...
Based on the $\mathcal{NP}$-hardness of \textsc{K-Partitioning}~\cite{babel1998thek} and the polynomial time to check a solution, we obtain the following proposition:
%
...
\subsubsection{Further \texorpdfstring{$\mathcal{NP}$}{NP}-Hardness Results}
\label{sec:afs:appendix:complexity:further-np-results}
...
Thereby, the choice of the key user-defined parameters for alternative selection, i.e.,~$a$, $k$, and~$\tau$, is severely limited.
While this particular problem structure resembles several well-known $\mathcal{NP}$-hard problems, it only is a special case of alternative feature selection and might not reflect many real-world scenarios.
In this section, we will show $\mathcal{NP}$-hardness for problem instances where these two assumptions do not hold.
We keep the focus on univariate feature qualities and the minimum-quality objective.
...
In the following, we will show that finding such an incomplete partitioning is $\mathcal{NP}$-complete in general.

Let us assume we have an arbitrary problem instance~$I$ of the complete-partitioning problem with $\tau=1$, i.e., zero overlap of feature sets.
Further, let the feature-set size~$k$ be fixed.
If we add one further feature~$f'$ to this problem instance and keep $a$ and $k$ as before, we obtain an instance~$I'$ of the incomplete-partitioning problem since one feature will not be selected.
We choose the quality~$q'$ of~$f'$ such that it is lower than the quality of all existing features in~$I$.
...
If~$f'$ is not selected, then this solutions of~$I'$ also solves~$I$.
However, if the qualities of the resulting alternative feature sets are not equal, $f'$ might still be chosen in a feature set that does not have the minimum quality of all feature sets since only the latter determines the overall objective value.
...
Since checking a solution is not costlier than for complete partitioning, we obtain membership in $\mathcal{NP}$ and thereby $\mathcal{NP}$-completeness.
...
In the following, we will show that allowing $0 < \tau < 1$ also results in an $\mathcal{NP}$-hard problem in general.
...
Again, let us assume we have an arbitrary problem instance~$I$ of the complete-partitioning problem with $\tau=1$.
Further, let the feature-set size~$k$ be fixed.
As before, define a new problem instance~$I'$ by adding a new feature~$f'$.
...
The goal is to force the selection of~$f'$ since any selection not involving it would be worse.
...
If we now obtain a solution for~$I'$, it contains~$f'$ in each feature set while the remaining features do not overlap.
Thus, we can remove~$f'$ to get feature sets of size~$k$ that constitute an optimal solution of the original problem~$I$.

This transformation shows how problem instances with $\tau < 1$ can help solve arbitrary problem instances with $\tau = 1$.
Given the $\mathcal{NP}$-completeness of the latter, we obtain $\mathcal{NP}$-hardness of the former:
%
...
Note that adding the proposed~$f'$ with a high quality~$q'$ clearly enlarges the size of the problem instance.
However, the proposed transformation from $I$ to $I'$ is still possible in polynomial time and increases the problem size by at most a fixed factor.
In particular, the encoding of a problem instance involves $n$~feature qualities as well as the values of $a$, $k$, $\tau$.
Let us assume the feature qualities in $I$ have an average encoding size of $c \in \mathbb{R}$, so the overall encoding size is $c \cdot n + O(1)$.
...
In consequence, the instance size of~$I'$ is at most roughly double the size of~$I$.

The above construction can be extended to all other $\tau > 0$.
...
The highest overlap, without allowing identical feature sets, requires creating an instance $I'$ with $\tau'= 1/k$ from an instance with $\tau = 1$.
...
\paragraph{Configuration of alternative feature selection}
...
Additionally, Propositions~\ref{prop:afs:complexity-partitioning-min-unconstrained-k} and~\ref{prop:afs:complexity-partitioning-min-constrained-k} required all features to be part of exactly one feature set, i.e., choosing the combination of~$a$ and~$k$ to match~$n$, which is another restriction that might not reflect real-world use cases.
...
For analyzing the previously discussed configurations of alternative feature selection, several questions spring to mind.
...
Further, while we placed alternative feature selection in the broad parameterized complexity class~$\mathcal{XP}$, one might proof membership in a more specific parameterized complexity class.

\paragraph{Related problem formulations}

We only focused on the quality-based optimization problem of alternative feature selection until now.
