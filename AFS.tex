...
\usepackage[letterpaper]{geometry} % SIAM wants letter format
\usepackage{ltexpprt} % required by SIAM template

\title{\Large Finding Optimal Solutions for Alternative Feature Selection} % \Large used in SIAM template

\date{} % don't display a date

\author{Jakob Bach\thanks{Karlsruhe Institute of Technology (KIT), Germany, \href{mailto:jakob.bach@kit.edu}{jakob.bach@kit.edu}}} % affiliation inserted via \thanks is used in SIAM template

\usepackage[style=ieee, backend=bibtex]{biblatex}
\usepackage[algo2e, ruled, linesnumbered, vlined]{algorithm2e} % pseudo-code; without option "algo2e", clashes with SIAM template
\usepackage{amsmath} % mathematical symbols
\usepackage{amssymb} % mathematical symbols
%\usepackage{amsthm} % theorems, definitions etc.; handled (overridden) by SIAM template
...
%\theoremstyle{definition} %handled by SIAM template
%\newtheorem{definition}{Definition}

%\newcommand\relatedversion{}
%\renewcommand\relatedversion{\thanks{The full version of the paper can be accessed at \protect\url{https://arxiv.org/abs/1902.09310}}} % command in SIAM template if there is another version

\fancyfoot[R]{\scriptsize{Copyright \textcopyright\ 2022\\ Copyright for this paper is retained by authors}}
...
\begin{abstract}
\small\baselineskip=9pt % used in SIAM template
Feature-selection methods are popular for obtaining small, interpretable, yet highly accurate prediction models.
...
\end{abstract}
\newline
\textbf{Keywords:} feature selection, alternatives, constraints, explainability, interpretability % no dedicated command in SIAM template
...
%
\begin{Definition}[Alternative feature selection]
	Given an original set of features, find a sufficiently different set that optimizes feature-set quality at the same time.
	\label{def:alternative-feature-selection}
\end{Definition}
%
We will provide formal definitions later.
Depending on how different the alternative feature set should be composed, one might have to compromise on quality.
E.g., if the original feature set already contains most of the highly predictive features, the alternative might have significantly lower quality.
...
In contrast, we target at different feature sets but similar feature-set quality, e.g., prediction performance.
...
Naturally, the exact decrease depends on the dataset and how feature-set quality is distributed.
...
Thus, this threshold allows the user to exercise control over alternatives and make use-case-specific choices.

We publish our code\footnote{temporary version for review: \url{https://www.dropbox.com/sh/s1n5vxfugtfurmg/AACecUHOzvrEq9EHg6KHXAvga?dl=0}} and our experimental data\footnote{temporary version for review: \url{https://www.dropbox.com/sh/3bmeoihgozmvfg3/AAD9AcRddqRVlu7tps6FxIKIa?dl=0}} online.
...
Note that we only give a short overview here.
...
Univariate filters assess each feature independently, while multivariate filters evaluate feature sets.
Examples of univariate filters are the Pearson correlation or the mutual information between a feature and the prediction target.
Such methods ignore the interaction between features, e.g., redundancies.
...
Wrappers can use an arbitrary black-box optimization strategy to iterate over feature sets, e.g., genetic algorithms.

\paragraph{Embedded methods}

Embedded methods have feature selection built into the training of prediction models, e.g., decision trees~\cite{breiman1984classification} or random forests~\cite{breiman2001random}.
Thus, the criterion to evaluate feature-set quality is model-specific.
Tree-based models typically use information gain or the Gini index.

\paragraph{Post-hoc feature-importance methods}

Apart from traditional feature selection, various methods assess feature importance after training a model.
...
In our article, we base dissimilarity on the Dice coefficient, a well-known and straightforward set-similarity measure:
%
\begin{equation}
...
A feature set that differs strongly might cause a significant drop in feature-set quality.
Thus, we leave $\tau$ as a parameter.
If the set-dissimilarity measure $d(\cdot)$ is normalized to $[0,1]$, like the Dice dissimilarity, the interpretation of $tau$ is user-friendly:
A value of $0$ allows the alternative feature set to be identical to the original one.
...
Overall, one can express alternative feature sets according to Definition~\ref{def:single-alternative} with 0-1 integer linear constraints.
...
We constrain the new feature set to be an alternative to all previously found feature sets:
%
\begin{Definition}[Sequential alternative]
	A feature set $F_2$ an alternative to a set of feature sets $\mathbb{F}$ (and vice versa) for a dissimilarity threshold $\tau \in \mathbb{R}_{\geq 0}$ if $\forall F_1 \in \mathbb{F}: d(F_1,F_2) \geq \tau$.
...
Regarding feature-set quality, we expect a drop with each alternative, as the solution space becomes narrower over iterations.
...
%
\begin{Definition}[Simultaneous alternative]
...
In this paper, we consider the summed quality of all feature sets as the objective.
...
Thus, the simultaneous procedure should yield higher average feature-set quality for the same number of alternatives.
...
In this section, we present concrete approaches for finding alternative feature sets.
...
If the feature-set quality function $Q(s,X,y)$ is sufficiently simple, one can tackle alternative feature selection with a white-box solver.
...
\end{equation}
%
Here, $q(\cdot)$ typically is a bivariate dependency measure, e.g., mutual information~\cite{kraskov2004estimating} or correlation, to quantify the relationship between a feature and the prediction target.
...
CFS~\cite{hall1999correlation} and mRMR~\cite{peng2005feature} allow white-box optimization as well~\cite{nguyen2014effective}.
...
%\end{multline}
%%
%Note that if one knows the feature-set size, the denominators are constant and thus, Equation~\ref{eq:mrmr} is linear if one replaces the product terms according to Equation~\ref{eq:product-linear}.
%Without this replacement, it is a quadratic-programming problem~\cite{rodriguez2010quadratic, nguyen2014effective}.
%
%\paragraph{FCBF}

FCBF bases on the notion of predominance:
It strives to select features with at least a certain correlation to the prediction target but a lower correlation to all other features.
...
With the constraint in Equation~\ref{eq:fcbf}, we enforce that if two features are selected, their correlation has to be lower than each feature's target correlation.
...
If feature-set quality has no closed-form expression, one can treat it as a black-box function when searching for alternative feature sets.
...
Third, one can formulate a multi-objective problem, so there are no hard constraints on alternatives anymore.
...
One idea is to prevent a search heuristic from producing feature sets that violate the constraints or at least make the latter less likely, e.g., by penalizing the objective function accordingly.
Another idea is to `repair' feature sets proposed by the search that are not alternative enough.
For example, one can replace a feature set violating the constraints with the most similar feature set satisfying the constraints.
Such solver-assisted search approaches are common in the search for software configurations~\cite{white2010automated,henard2015combining,guo2018preserve}.
...
First, the algorithm uses a solver to find one sufficiently alternative solution.
...
Each iteration corresponds to one solver call and, if the solver yields a valid feature set, one training of the prediction model.
...
In this section, we review literature from areas related to alternative feature selection:
...
\cite{bailey2014alternative} also distinguishes between singular alternatives and multiple alternatives, found sequentially or simultaneously.
...
%MAXIMUS~\cite{bae2010clustering} employs an integer program to formulate dissimilarity between clusterings, building on the distributions of feature values in clusters.
%The solutions to this problem are not concrete alternative clusterings but constrain a subsequent clustering procedure.
...
Some subspace approaches explicitly try to remove redundancy between subspaces~\cite{mueller2009relevant, nguyen20134s} or foster subspace diversity~\cite{trittenbach2019dimension, fouche2021efficient}, which bears some resemblance to alternative feature selection.
Nevertheless, the scenarios are fundamentally different, e.g., we are in a supervised setting.
...
\cite{leeuwen2012diverse} presents six strategies to foster diversity in subgroup set discovery, which searches for interesting regions in the data space rather than explicitly selecting features.
...
Thus, this focus is different from ours.
...
Depending on the configuration of statistical tests, there can be an arbitrary number of equivalent feature sets, without an explicit ordering.
...
Our dissimilarity threshold allows controlling the overlap of feature sets instead of eliminating all feature redundancies.
Nevertheless, if one wants to prevent the selection of certain redundant features or feature sets reliably, one can achieve this with additional constraints in our optimization problem as well.
...
We conduct experiments with 30 datasets for binary classification.
...
For univariate filtering, we use Equation~\ref{eq:univariate-filter} with mutual information~\cite{kraskov2004estimating} as dependency measure $q(\cdot)$.
...
As a wrapper method, we employ a greedy hill-climbing strategy according to Algorithm~\ref{al:greedy-wrapper}.
...
As a post-hoc importance measure, we take model-based feature importance provided by \emph{scikit-learn}.
...
Solver runtime may vary considerably even for optimization problems of the same size, so we set a timeout of 60~s on each solver run.
...
We mainly use the default hyperparameters of the models, apart from choosing information gain as the split criterion, to be consistent with filter feature selection, and setting a random seed for reproducibility.
...
To express and solve the constraint optimization problems, we use the solver \emph{CBC} via the package \emph{OR-Tools}~\cite{or-tools}.
...
For example, the distribution of feature-set quality might be relatively uniform or relatively skewed.
...
This observation is expected as we do not regularize or prune the trees.
However, this will not invalidate our analysis of how prediction performance develops over alternatives, where we normalize feature-set quality.
...
\begin{figure*}[t]
	...
	\subfloat[Training set.]{
		\includegraphics[width=0.99\columnwidth, trim=0 15 0 15, clip]{plots/impact-num-alternatives-train-objective-tau.pdf}
		\label{fig:impact-num-alternatives-train-objective-tau}
	}
	\hfill
	\subfloat[Test set.]{
		\includegraphics[width=0.99\columnwidth, trim=0 15 0 15, clip]{plots/impact-num-alternatives-test-objective-tau.pdf}
...
As expected, the training-set objective value $Q$ of feature sets within a particular search run varies more for sequential than for simultaneous search.
...
I.e., simultaneous search finds more homogeneous feature sets, particularly if many alternatives are desired.
...
Regarding test-set prediction performance, simultaneous search does not even show lower variance than sequential search anymore.
...
For up to three alternatives in simultaneous search, less than 1\% of feature sets are potentially sub-optimal, but 4\% of the feature sets are for four alternatives and 32\% of the feature sets for five alternatives.
...
As feature-set quality varies between datasets and the number of features $k$, we apply normalization:
...
Additionally, Figure~\ref{fig:impact-num-alternatives-objective} shows that the test-set objective value also drops most to the first alternative.
...
This outcome also depends on the dissimilarity threshold $\tau$, which we analyze next.
...
Also, the number of infeasible optimization problems, i.e., lack of valid alternative feature sets, increases with $\tau$.

While the prior observations were made for \emph{MI} as the feature selector, they do not hold universally.
...
For \emph{Greedy Wrapper}, this can be explained by its heuristic search.
...
\balance % according to documentation, command might not work if issued too late in document, so hopefully placing it here suffices
...
Such alternative feature sets might provide alternative explanations for predictions.
...
Further, we presented approaches to solve the optimization problem for different categories of feature selectors.
...
In our experiments, we found that one can obtain alternative feature sets with high feature-set quality.
...
For example, one can integrate other feature-selection methods, particularly embedded methods.
