...
The feature selection $s^{(0)} = (0,0,0,1,1)$, $s^{(1)} = (0,1,1,0,0)$ yields the optimal summed quality of $9+5=14$, while the quality of the two feature sets differs significantly.
...
%
\begin{equation}
	\max_{s^{(0)}, \dots, s^{(a)}} \min_{i \in \{0, \dots, a\}} Q(s^{(i)},X,y)~\left(\text{rather than:} \max_{s^{(0)}, \dots, s^{(a)}} \sum_{i=0}^a Q(s^{(i)},X,y) \right)
...
For an objective consisting of univariate feature qualities (cf.~Equation~\ref{eq:univariate-filter} and Section~\ref{sec:appendix:complete-optimization-problem}), one can easily find a certain number of optimal alternative feature sets without a solver.
To this end, we propose the following \emph{greedy-replacement} procedure:
The mentioned objective function does not contain interaction terms between features.
Thus, one should sort the features decreasingly by their individual qualities~$q(\cdot)$.
The optimal feature set of size $k$ simply consists of the top~$k$ features from this ordering.
Knowing the feature-set size $k$ and using a simple set-dissimilarity measure like the Dice dissimilarity, one can determine how many features needs to differ for a valid alternative (cf.~Equation~\ref{eq:dice-rearranged-equal-size}), i.e., $\tau \cdot k$ features.
Thus, one should pick this number of features from the current feature set, choosing the features with the lowest qualities, and replace them with the highest-quality features that were not part of any feature set yet.
To obtain multiple alternatives, one can continue this procedure iteratively.
...
The seconds alternative consists of the three best features plus the eight- and ninth-best feature.
In general, the $(1 - \tau) \cdot k$ best features plus the $k + (i-1) \cdot \tau \cdot k + 1$ to $k + i \cdot \tau \cdot k$ best features form the $i$-th alternative.

The summed quality of the solutions yielded by this greedy sequential procedure cannot even be surpassed by simultaneous search, though the latter might compose the feature sets differently.
However, greedy replacement only works as long as some features have not been part of any feature set yet, i.e., $k + i \cdot \tau \cdot k \leq n$.
Once this pool of unused features is exhausted, one would need another strategy to find further alternatives.
From that point on, a continuation of the sequential procedure might perform worse than a simultaneous search.

Obtaining a high number of alternatives from greedy replacement favors scenarios with a high number of features $n$, a small feature-set size $k$, and a low dissimilarity threshold~$\tau$.
These conditions align well with classical feature-selection scenarios where~$k \ll n$.

Beside the drawback of potentially running out of unused features, there are further disadvantages.
In particular, greedy replacement does not work once the optimization problem gets more involved.
For example, there might be further constraints on feature sets, e.g,, based on domain knowledge.
Also, the objective function becomes more complex for other feature selectors than univariate filters.
Simultaneous search with the objective from Appendix~\ref{sec:appendix:min-quality-objective} also is unsuitable for greedy replacement.
