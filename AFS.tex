...
The parallelized pipeline run took roughly 255~hours, i.e., about 10.6 days.
...
Roughly 2-3\% of the searches for \emph{FCBF}, \emph{MI}, and \emph{Model Gain}, and 67\% of the searches for \emph{mRMR} found a feasible solution but could not proof optimality.
Such timeout-affected simultaneous solutions can be worse than optimal sequential solutions.
The optimization status \emph{not solved}, i.e., not finding a feasible solution without proofing infeasibility, did not occur in the displayed results.
...
Further, \emph{FCBF}, \emph{MI}, and \emph{Model Gain} experiences a dramatic increase in optimization time with the number of alternatives~$a$ in simultaneous search, as Table~\ref{tab:afs:impact-num-alternatives-fs-method-optimization-time} displays.
...
Besides factors related to the datasets and experimental design, the heuristics used by the solver may also cause runtime to fluctuate considerably.
...
The median test-set MCC of decision trees is 0.60 for \emph{Model Gain}, 0.53 for \emph{FCBF}, 0.54 for \emph{MI}, 0.54 for \emph{mRMR}, and 0.51 for \emph{Greedy Wrapper}.
...
This method can only evaluate a fraction of all feasible feature sets while the remaining feature-selection methods optimize globally.
...
However, the number of possible feature sets is much higher, e.g., $2^{15}=32768$ for the lowest-dimensional datasets in our evaluation (cf.~Table~\ref{tab:afs:datasets}).
