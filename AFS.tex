...
Section~\ref{sec:afs:appendix:univariate-search} proposes search heuristics for univariate feature selection (cf.~Equation~\ref{eq:afs:univariate-filter}).
...
Further, there might be efficient exact or approximate algorithms for problem instances satisfying additional assumptions.
Finally, while we placed alternative feature selection in class~$\mathcal{XP}$ (cf.~Proposition~\ref{prop:afs:complexity-simultaneuos-xp}), one might prove membership or hardness for more specific parameterized classes.
...
The second search approach is simultaneous, the other two are sequential.

\subsubsection{Greedy Replacement Search}
\label{sec:afs:appendix:univariate-search:greedy-replacement}

\emph{Greedy Replacement Search}, is the first heuristic we propose for alternative feature selection with the univariate objective.
This heuristic conducts a sequential search.
We build on the fact that the univariate objective does not contain interaction terms between features.
Thus, we can assess the quality of each feature independently.
...
We start by sorting the features decreasingly according to their individual qualities~$q_j$ (Line~\ref{al:afs:greedy-replacement:line:sorting}).
For a fixed feature-set size~$k$, a dissimilarity threshold~$\tau$, and using the Dice dissimilarity, $\lceil \tau \cdot k \rceil$~features need to differ between feature sets (cf.~Equation~\ref{eq:afs:dice-rearranged-equal-size}).
In other words, the same subset of $\lfloor (1 - \tau) \cdot k \rfloor$~features can be contained in all alternatives without violating the dissimilarity threshold.
...
For each alternative, we select the $\lceil \tau \cdot k \rceil$~highest-quality features that are not part of any prior alternative.
We continue this procedure till we reach the desired number of alternatives~$a$ or till there are not enough unused features to form further alternatives (Line~\ref{al:afs:greedy-replacement:line:stop}).
%
\begin{example}
...
The original feature set consists of the top five features regarding quality~$q(\cdot)$.
The first alternative consists of the top three features plus the sixth- and seventh-best feature.
The second alternative consists of the top three features plus the eight- and ninth-best feature.
After that alternative, the heuristic has to stop, as there are not enough unused features to form further alternatives.
\label{ex:afs:greedy-replacement:algorithm}
\end{example}
%
In general, the top $\lfloor (1 - \tau) \cdot k \rfloor$ features plus the features with positions $k + (i-1) \cdot \lceil \tau \cdot k \rceil + 1$ to $k + i \cdot \lceil \tau \cdot k \rceil$ in a descending quality ordering form the $i$-th alternative.
...
In particular, greedily assembling feature sets of different sizes in different order can result in a different overall objective value, i.e., summed feature-set quality.

\paragraph{Complexity}

Sorting the individual qualities of all~$n$ features (Line~\ref{al:afs:greedy-replacement:line:sorting}) has a complexity of $O(n \cdot \log n)$.
After that, the algorithm iterates over the features and processes each feature at most once.
In particular, after adding a feature to an alternative, $feature\_position$ increases by~1.
This variable cannot exceed the total number of features~$n$.
The actual number of features processed depends on the parameters~$a$ and~$k$ (Line~\ref{al:afs:greedy-replacement:line:stop}).
...
Combining the cost per $feature\_position$ with the total number of $feature\_position$s, the overall time complexity is~$O(n^2)$, i.e., polynomial in~$n$.

\paragraph{Quality}

\emph{Greedy Replacement Search} includes the top $\lfloor (1 - \tau) \cdot k \rfloor$ features regarding quality in each alternative of size~$k$.
In the univariate objective, the quality of a feature set is the sum of the qualities of the contained features.
Hence, assuming non-negative qualities, each alternative yielded by this heuristic has at least a quality of $\lfloor (1 - \tau) \cdot k \rfloor / k$ compared to the optimal feature set of size~$k$, even if the remaining $\lceil \tau \cdot k \rceil$ features selected by the heuristic had a quality of~0.
Further, the quality of the optimal feature set of size~$k$ upper-bounds the quality of any feature set of size~$k$ found by any search method.
Thus, we obtain the following proposition:
%
\begin{proposition}[Quality of \emph{Greedy Replacement Search}]
	...
	Under these conditions, \emph{Greedy Replacement Search} reaches at least a fraction of $\lfloor (1 - \tau) \cdot k \rfloor / k$ of the objective values of (1) sequential search, (2) simultaneous search with sum-aggregation, and (3) simultaneous search with min-aggregation.
	\label{prop:afs:approximation-greedy-replacement}
\end{proposition}

In particular, \emph{Greedy Replacement Search} yields a constant-factor approximation of the three mentioned optimization problems (cf.~Equation~\ref{eq:afs:afs-sequential} and~\ref{eq:afs:afs-simultaneous}).
Given the polynomial runtime of this heuristic, we obtain the following proposition:
%
\begin{proposition}[Approximation complexity of alternative feature selection]
	Assume non-negative univariate feature qualities of $n$~features, $a \in \mathbb{N}_0$~alternatives, a dissimilarity threshold~$\tau$, desired feature-set size~$k$, and $k + a \cdot \lceil \tau \cdot k \rceil \leq n$.
	Under these conditions, the problems of (1) sequential search, (2) simultaneous search with sum-aggregation, and (3) simultaneous search with min-aggregation for alternative feature sets reside in the complexity class~$\mathcal{APX}$.
	\label{prop:afs:approximation-apx}
\end{proposition}
%
The following two examples show that the heuristic can be worse than exact search for $a=2$ with sum-aggregation and $a=1$ with min-aggregation.
%
\begin{example}
Consider $n=6$~features with univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter}) $q = (9,8,7,3,2,1)$, feature-set size~$k=2$, number of alternatives~$a=2$, and dissimilarity threshold~$\tau = 0.5$, which permits an overlap of one feature between sets here.
Sequential search and simultaneous search, for both types of objective aggregation, yield the selection $s^{(0)} = (1,1,0,0,0,0)$, $s^{(1)} = (1,0,1,0,0,0)$, and $s^{(2)} = (0,1,1,0,0,0)$, with a summed quality of $17+16+15=48$.
...
%
\begin{example}
Consider $n=6$~features with univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter}) $q = (9,8,7,3,2,1)$, feature-set size~$k=3$, number of alternatives~$a=1$, and dissimilarity threshold~$\tau = 0.5$, which permits an overlap of one feature between sets here.
Simultaneous search with min-aggregation yields the selection $s^{(0)} = (1,1,0,0,0,0)$ and $s^{(1)} = (1,0,1,0,0,0)$, with quality of $\min~\{19,19\} = 19$.
\emph{Greedy Replacement Search} yields the selection $s^{(0)} = (1,1,1,0,0,0)$ and $s^{(1)} = (1,0,0,1,1,0)$, with quality of $\min~\{19,14\} = 14$.
\label{ex:afs:greedy-replacement:worse-than-min-agg}
\end{example}
%
Sequential search and simultaneous search with sum-aggregation would perform no better than the heuristic here.
The issue is that the sequential heuristic does not attempt to balance feature-set qualities.
We alleviate this issue by introducing the heuristic~\emph{Greedy Balancing Search} (cf.~Section~\ref{sec:afs:appendix:univariate-search:greedy-balancing}).

\paragraph{Limitations}

Proposition~\ref{prop:afs:approximation-greedy-replacement} and Examples~\ref{ex:afs:greedy-replacement:worse-than-sum-agg},~\ref{ex:afs:greedy-replacement:worse-than-min-agg} already showed the potential quality loss of the heuristic compared to exact search for alternatives.
...
Thus, to obtain a high number of alternatives~$a$ with greedy replacement, the following conditions are beneficial:
...
Once \emph{Greedy Replacement Search} runs out of unused features, one would need to apply another search method.

Another drawback is that \emph{Greedy Replacement Search} assumes a very simple structure of the optimization problem.
If the objective function becomes more complex than for univariate filter feature-selection, the quality-based feature ordering becomes impossible or suboptimal.
...
Also, \emph{Greedy Replacement Search} cannot accommodate further constraints on feature sets, e.g., based on domain knowledge.

Given the limitation of \emph{Greedy Replacement Search} and the low optimization time for sequential search with the univariate objective (cf.~Table~\ref{tab:afs:impact-search-fs-method-optimization-time}), we only employ exact optimization with a solver in our experiments.

\subsubsection{Greedy Balancing Search}
\label{sec:afs:appendix:univariate-search:greedy-balancing}

\emph{Greed Balancing Search} modifies \emph{Greedy Replacement Search} such that the feature-set qualities become more balanced.
The resulting search heuristic is simultaneous rather than sequential.
...
First, we check whether the algorithm should terminate early, in case the number of features~$n$ is not high enough for alternatives satisfying the desired user parameters~$k$, $a$, and~$\tau$ (Line~\ref{al:afs:greedy-balancing:line:stop-early}).
Then, we select the first $\lfloor (1 - \tau) \cdot k \rfloor$~features in each alternative in the same way as \emph{Greedy Replacement Search} (cf.~Algorithm~\ref{al:afs:greedy-replacement}:
We pick the features with the highest quality~$q_j$ (Lines~\ref{al:afs:greedy-balancing:line:common-features-start}--\ref{al:afs:greedy-balancing:line:common-features-end}).

For the remaining spots in the alternative, we use a Longest Processing Time (LPT) heuristic (Lines~\ref{al:afs:greedy-balancing:line:disjoint-features-start}--\ref{al:afs:greedy-balancing:line:disjoint-features-end}).
This type of heuristic is common for \emph{Multiprocessor Scheduling} and \textsc{Balanced Number Partitioning} problems~\cite{babel1998thek, chen20023partitioning, lawrinenko2018reduction} (cf.~Section\ref{sec:afs:appendix:complexity:related-work}).
...
We continue this procedure until all alternatives have reached their size (Line~\ref{al:afs:greedy-balancing:line:stop}).
%
\begin{example}
Consider $n=6$~features with univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter}) $q = (9,8,7,3,2,1)$, feature-set size~$k=4$, number of alternatives~$a=1$, and dissimilarity threshold~$\tau = 0.5$, which permits an overlap of two features between sets here.
The features with qualities~$9$ and $8$ become part of $s^{(0)}$ and $s^{(1)}$ (Lines~\ref{al:afs:greedy-balancing:line:common-features-start}--\ref{al:afs:greedy-balancing:line:common-features-end}).
At this point, both alternatives have the same relative quality $Q^{(0)} = Q^{(1)} = 0$, i.e., the algorithm ignores the quality of the shared features.
...
The feature with quality~$7$ is added to $s^{(0)}$.
Since this causes $Q^{(0)} > Q^{(1)}$, the feature with quality~3 is added to $s^{(1)}$.
As $Q^{(0)} > Q^{(1)}$ still holds, the feature with quality~2 becomes part of $s^{(1)}$ as well.
Because $s^{(1)}$ has reached size~$k$, the feature with quality~1 is added to $s^{(0)}$, even if the latter still has higher quality.
Now both alternatives have reached their desired size and $n = 6 = \lceil 0.5 \cdot 4 \rceil \cdot 1 + 2 = \lceil \tau \cdot k \rceil \cdot a + k$ (Line~\ref{al:afs:greedy-balancing:line:stop}).
...
Like \emph{Greedy Replacement Search}, \emph{Greedy Balancing Search} has an upfront cost of $O(n \cdot \log n)$ for sorting feature qualities (Line~\ref{al:afs:greedy-balancing:line:sorting}) and then iterates over each of $O(n)$ $feature\_position$ exactly once.
For each $feature\_position$, the algorithm iterates over the $a$~alternatives and conducts a fixed number of array operations in at most $O(n)$.
Thus, the overall complexity of \emph{Greedy Balancing Search} is $O(a \cdot n^2)$.

\paragraph{Quality}

\emph{Greed Balancing Search} uses same features as \emph{Greedy Replacement Search} and only changes their distribution into the feature sets.
Thus, the summed feature-set quality remains the same, while the minimum feature-set quality may be higher.
...
%
\begin{proposition}[Quality of \emph{Greedy Balancing Search}]
	Assume non-negative univariate feature qualities of $n$~features, $a \in \mathbb{N}_0$~alternatives, a dissimilarity threshold~$\tau$, desired feature-set size~$k$, and $k + a \cdot \lceil \tau \cdot k \rceil \leq n$.
	Under these conditions, \emph{Greedy Balancing Search} reaches at least a fraction of $\lfloor (1 - \tau) \cdot k \rfloor / k$ of the objective values of (1) sequential search, (2) simultaneous search with sum-aggregation, and (3) simultaneous search with min-aggregation.
	\label{prop:afs:approximation-greedy-balancing}
\end{proposition}
%
As the following examples shows, \emph{Greedy Balancing Search} can be better than sequential search regarding the objective with min-aggregation, already for $a=1$:
%
\begin{example}
	Consider $n=6$~features with univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter}) $q = (9,8,7,3,2,1)$, feature-set size~$k=3$, number of alternatives~$a=1$, and dissimilarity threshold~$\tau = 0.5$, which permits an overlap of one feature between sets here.
	Sequential search yields the selection $s^{(0)} = (1,1,1,0,0,0)$ and $s^{(1)} = (1,0,0,1,1,0)$, with quality of $\min~\{24,14\} = 14$.
	\emph{Greedy Balancing Search} and simultaneous search with min-aggregation yield the selection $s^{(0)} = (1,1,0,0,1,0)$ and $s^{(1)} = (1,0,1,1,0,0)$, with quality of $\min~\{19,19\} = 19$.
	Simultaneous search with sum-aggregation may yield both solutions.
	\label{ex:afs:greedy-balancing:better-than-sequential}
\end{example}
%
...
\emph{Greedy Balancing Search} shares several limitations with \emph{Greedy Replacement Search}:
It may be worse than exact search, assumes a very simple notion of feature-set quality, and does not work if the number of features is too low relative to~$k$, $a$, and $\tau$.
In the latter case, \emph{Greedy Replacement Search} yields no solution at all due to its simultaneous nature, while \emph{Greedy Balancing Search} yields at least some solutions.
...
\emph{Greedy Depth Search} is a sequential search heuristic that generalizes \emph{Greedy Replacement Search}.
In particular, the former allows to progress beyond $\frac{n - k}{\lceil \tau \cdot k \rceil}$ alternatives.
...
\begin{algorithm}[tp]
	...
	\While{$i \leq a$ \textbf{and} $has\_next\_solution$}{ \label{al:afs:greedy-depth:line:main-loop-start} \label{al:afs:greedy-depth:line:stop}
		...
		\If(\tcp*[f]{Updating positions further would violate $n$}){$p = 0$}{
...
As in the other two heuristics, we start by sorting the features decreasingly according to their individual qualities~$q_j$ (Line~\ref{al:afs:greedy-replacement:line:sorting}).
However, instead of keeping the same $\lfloor (1 - \tau) \cdot k \rfloor$~features in each alternative and only replacing the remaining ones, we now allow all positions in the current alternative to change features.
Thus, we maintain an array of the feature positions in the current alternative, i.e., indices regarding the sorted qualities (Lines~\ref{al:afs:greedy-depth:line:position-init-start}--\ref{al:afs:greedy-depth:line:position-init-end}).
...
For each alternative, we select the features according to the position array (Lines~\ref{al:afs:greedy-depth:line:selection-start}--\ref{al:afs:greedy-depth:line:selection-end}).
...
This check was unnecessary in the previous two heuristics, which only form valid alternatives by design.

Next, we update the positions of the selected features in the current alternative. (Lines~\ref{al:afs:greedy-depth:line:position-update-start}--\ref{al:afs:greedy-depth:line:position-update-end}).
At first, we try to decrease the quality of the lowest-quality feature in the current alternative by advancing one position regarding the sorted qualities.
This step may not be possible, as the alternative may already contain the feature with the overall lowest quality, i.e., position~$n$ in the array of sorted qualities (Line~\ref{al:afs:greedy-depth:line:check-position-update}).
In this case, we try to decrease the quality of the second-lowest-quality feature in the current alternative by one position.
If this action is impossible as well, we advance further over the features in the alternative by increasing quality (Line~\ref{al:afs:greedy-depth:line:continue-prior-position}).
Once we find a feature whose quality we can decrease, we also decrease the quality of all lower-quality features accordingly, so the feature positions to form alternatives remain sorted by decreasing quality (Lines~\ref{al:afs:greedy-depth:line:quality-decrease-start}--\ref{al:afs:greedy-depth:line:quality-decrease-end}).

We repeat the overall main loop till we reach the desired number of alternatives~$a$ or till we cannot update any feature position without exceeding the number of features~$n$, i.e., we cannot form a further alternative (Line~\ref{al:afs:greedy-depth:line:stop}).
%
\begin{example}
Consider $n=6$~features with univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter}) $q = (9,8,7,3,2,1)$, feature-set size~$k=4$, number of alternatives~$a=1$, and dissimilarity threshold~$\tau = 0.5$, which permits an overlap of two features between sets here.
...
Thus, the algorithm initializes $feature\_positions = (1,2,3,4)$ (Line~\ref{al:afs:greedy-depth:line:position-init-start}--\ref{al:afs:greedy-depth:line:position-init-end}).
...
However, neither of these feature sets yield a valid alternative.
Thus, the algorithm attempts to replace the feature with the second-lowest quality as well, starting with $feature\_positions = (1,2,4,5)$ and $feature\_positions = (1,2,4,6)$.
However, the overlap with $s^{(0)}$ is still too large.
The next value in line is $feature\_positions = (1,2,5,6)$, which yields the valid alternative $s^{(1)} = (1,1,0,0,1,1)$.

\emph{Greedy Replacement Search} would terminate here since the options for only replacing the two lowest-quality features in the alternative are exhausted.
In contrast, \emph{Greedy Depth Search} now attempts to replace the third-lowest-quality feature, starting with $feature\_positions = (1,3,4,5)$.
...
Eventually, the algorithm also replaces the highest-quality feature, starting with $feature\_positions = (2,3,4,4)$.
Eventually, the algorithm reaches $feature\_positions = (3,4,5,6)$, which yields the valid alternative $s^{(2)} = (0,0,1,1,1,1)$.
After obtaining this solution, there is no valid update of the feature positions left (Line~\ref{al:afs:greedy-depth:line:position-update-end}).
Thus, the algorithm terminates.
...
For each values of $feature\_positions$, the algorithm creates a feature selection in $O(k^2 \cdot n)$ (Lines~\ref{al:afs:greedy-depth:line:selection-start}--\ref{al:afs:greedy-depth:line:selection-end}), checks constraints in $O(a \cdot n)$ (Line~\ref{al:afs:greedy-depth:line:check-valid}), and updates the position array (lines~\ref{al:afs:greedy-depth:line:position-update-start}--\ref{al:afs:greedy-depth:line:position-update-end}) in~$O(k^2)$.
In the worst case, \emph{Greedy Depth Search} finishes after $O(n^k)$ feature sets when it has exhausted its iteration over $feature\_positions$.
However, due to the sequential nature of the heuristic, users can interrupt it at any time and retain the alternatives found so far.
Also, \emph{Greedy Depth Search} can yield more alternatives than \emph{Greedy Replacement Search} with its $O(n^2)$ cost or \emph{Greedy Balancing Search} with its $O(a \cdot n^2)$ cost.
Nevertheless, in scenarios were the latter two are applicable, i.e., $k + a \cdot \lceil \tau \cdot k \rceil \leq n$, they have a lower cost than \emph{Greedy Depth Search}.
In particular, \emph{Greedy Depth Search} needs $O(n^{\lceil \tau \cdot k \rceil})$~iterations to cover all options of replacing of the worst $\lceil \tau \cdot k \rceil$ features in the size-$k$ feature sets, which is the search space of \emph{Greedy Replacement Search}.

\paragraph{Quality}

\emph{Greedy Depth Search} initially yield the same solutions as \emph{Greedy Replacement Search} and then continues to provide solutions where \emph{Greedy Replacement Search} stops.
...
For further alternatives, a quality analysis becomes more involved, because the alternatives do not contain all top $\lfloor (1 - \tau) \cdot k \rfloor$~features anymore. Thus, we leave this analysis open for future work.

\paragraph{Limitations}

Like \emph{Greedy Balancing Search} and \emph{Greedy Replacement Search}, \emph{Greedy Depth Search} assumes a very simple notion of feature-set quality and may be worse than exact search.
It may yield more alternative than the former two heuristics but has a higher and more variable runtime behavior, as discussed before.
