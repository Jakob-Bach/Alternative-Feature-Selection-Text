...
\begin{figure}[p]
	...
	\end{subfigure}
	\caption{
		Mean of feature-set quality, max-normalized per search run for alternatives, over the number of alternatives and dissimilarity threshold~$\tau$, by evaluation metric and normalization method.
...
Finally, one could employ a heuristic rather than an exact search method (cf.~Section~\ref{sec:afs:approach:univariate-heuristics}).
%TODO heuristics (we already did that)
