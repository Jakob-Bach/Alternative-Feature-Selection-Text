...
Additionally, note that Proposition~\ref{prop:afs:complexity-exhaustive-simultaneuos} assumes that constraint checking does not require linearization variables like $t^{(i_1,i_2)}_j$ in Equation~\ref{eq:afs:afs-simultaneous-complete}, which would increase the search space even further.
...
In particular, assuming $(a + 1) \cdot k < n$, it is sufficient to use the $(a + 1) \cdot k$ highest feature qualities instead of all features when searching for $a + 1$ feature sets.
Due to monotonicity, the remaining feature qualities are irrelevant and might be dropped in a preprocessing step since they cannot improve the objective.
Assuming sufficiently small, constant $a$ and $k$ again, i.e., $a \cdot k \in O(1)$, the number of relevant feature-set qualities and thereby the pure search effort becomes independent from~$n$, which is even better than the polynomial runtime for general objectives under that assumption (cf.~Section~\ref{sec:afs:appendix:complexity:exhaustive}).
...
In this section, we show $\mathcal{NP}$-completeness for problem instances where these two assumptions do not hold.
...
In the following, we show that the problem of finding such an incomplete partitioning is $\mathcal{NP}$-complete in general.
...
Since checking a solution is possible in polynomial time, we obtain membership in $\mathcal{NP}$ and thereby $\mathcal{NP}$-completeness.
...
In the following, we show that allowing $0 < \tau < 1$ also results in an $\mathcal{NP}$-complete problem in general.
...
Given the $\mathcal{NP}$-completeness of the latter problem and membership in $\mathcal{NP}$ of the former, we obtain an $\mathcal{NP}$-completeness result:
%
\begin{proposition}[Complexity of simultaneous search with minimum-quality objective, $\tau < 1$, and constrained feature-set size]
	Assuming univariate feature qualities, a dissimilarity threshold~$\tau < 1$, and desired feature-set size~$k$, simultaneous search for $a + 1$~alternative feature sets with the minimum-quality objective is $\mathcal{NP}$-complete.
	\label{prop:afs:complexity-no-partitioning-min-constrained-k}
\end{proposition}

Note that adding the proposed~$f'$ with a high quality~$q'$ enlarges the size of the problem instance considerably.
...
In contrast to the hardness results for the minimum-quality objective, the summed quality objective (cf.~Equation~\ref{eq:afs:afs-simultaneous-sum-objective}) with univariate feature qualities and $\tau=1$ allows for polynomial-time algorithms.
In particular, this objective does not entail any requirement on balancing the feature sets' qualities, which eases finding an optimal solution for~$\tau=1$.
...
Using the monotonicity of the objective (cf.~Section~\ref{sec:afs:appendix:complexity:uni}), we order the features decreasingly by their individual quality and simply pick features without replacement until we have the desired number of alternatives with the desired feature-set sizes.
Again, it does not matter for the objective value how the selected feature are assigned to individual feature sets.
Due to the quality-based sorting, the time complexity is~$O(n \cdot \log n)$, i.e., polynomial.
...
For example, though we obtained $\mathcal{NP}$-completeness for the minimum-quality-objective with feature-set overlap (cf.~Proposition~\ref{prop:afs:complexity-no-partitioning-min-constrained-k}, an analysis of the summed-quality-objective with overlap is open.
We do have polynomial runtime for that objective subject to $\tau=1$ (cf.~Proposition~\ref{prop:afs:complexity-partitioning-sum}) but this result might not extend to~$\tau < 1$.
In particular, $\tau < 1$ increases the number of valid solutions, which could make searching the optimum more expensive.
...
Finally, while we placed alternative feature selection in the broad parameterized complexity class~$\mathcal{XP}$, one might proof membership in a more specific parameterized complexity class.
...
Also, given the number of alternatives, it would be interesting to have an exact or approximate estimate for the number of valid solutions for alternative feature selection, i.e., sets of feature sets.
