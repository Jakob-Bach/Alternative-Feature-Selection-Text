...
Existing feature-selection methods typically yield only one feature set, which might not suffice in some scenarios.
...
In our experiments, we observe that alternatives might indeed have similar prediction quality as the original feature sets, though several factors influence this outcome.
\end{abstract}
%
...
By reducing dataset dimensionality, they lower the prediction models' computational and memory requirements.
...
Third, the user might be interested in explanations for predictions rather than just making good predictions.
In such a scenario, knowing alternative feature sets allows the formulation of multiple, diverse explanations.
...
We use 30 classification datasets from the Penn Machine Learning Benchmarks (PMLB)~\cite{olson2017pmlb, romano2021pmlb} and five feature-selection methods.
...
Thereby, several factors influence the quality of alternatives, i.e., the dataset, feature-selection method, notion of feature-set quality, and parameters for searching alternatives.
...
Also, depending on the number of features in the dataset, no valid alternatives might exist if the parameter values are too strict.
Computationally, a sequential search for alternatives was significantly faster than searching for multiple alternatives simultaneously while yielding a similar quality.
Finally, we publish our code\footnote{\url{https://github.com/Jakob-Bach/Alternative-Feature-Selection}} and our experimental data\footnote{\url{https://www.dropbox.com/sh/3bmeoihgozmvfg3/AAD9AcRddqRVlu7tps6FxIKIa?dl=0}} online. % TODO

\paragraph{Outline}

Section~\ref{sec:afs:fundamentals} introduces the fundamentals of feature selection.
...
In fact, one can replace the product $s'_j \cdot s''_j$, which is non-linear, with linear constraints by introducing an auxiliary variable $t_j$ as follows~\cite{mosek2022modeling}:
%
\begin{equation}
	\begin{aligned}
		t_j \leq& s'_j \\
...
If such a solution~$s'$ exists and its quality~$Q(s',X,y)$ improves the current solution, the algorithm continues from the new solution and tries again to swap the first feature.
...
\begin{figure}[htb]
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\caption{Correlation between evaluation metrics, first computed per feature-selection method and then averaged.}
...
Since the different feature-selection methods have different objective functions, we computed the correlation for each feature-selection method separately and then averaged over them.
...
In particular, \emph{Greedy Wrapper} has a correlation of 0.88 between training-set objective value and test-set prediction performance.
Since this feature-selection method uses prediction models for the objective value, the correlation is high.
On the other hand of the spectrum, \emph{MI} with its simple univariate objective exhibits a correlation of -0.32 between training-set objective value and test-set prediction performance.
...
We formalized alternatives via constraints that are independent of the feature-selection method, can be combined with other constraints on feature sets, and allow users to control alternatives according to their needs.
