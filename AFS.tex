...
We only require users to set two parameters, i.e., the number of alternatives and a dissimilarity threshold for feature sets being alternative.
In fact, these parameters allow users to control the search for alternatives according to their needs.
...
Our notion of alternatives is general and thus independent of specific feature-selection methods.
...
Only users can decide which drop in feature-set quality is acceptable for them as a tradeoff for obtaining an alternative feature set.
...
Continue this procedure till an alternative with an acceptable quality-dissimilarity tradeoff is found.
...
In the field of XAI, alternative explanations might provide additional insights about predictions, allow to develop and test different hypotheses, appeal to different kinds of users, and foster trust into the predictive system~\cite{bansal2021does, kim2021multi, wang2019designing}.
In contrast, significantly different explanations for the same prediction might also raise doubts how meaningful the explanations are~\cite{jain2019attention}.
Finding diverse explanations is an issue for different explainers, e.g., for counterfactuals~\cite{dandl2020multi, guidotti2022counterfactual, mothilal2020explaining, russell2019efficient}, criticisms~\cite{kim2016examples}, or semifactual explanations~\cite{artelt2022even}.
One can also ensemble different kinds of explanations~\cite{silva2019produce}.

The idea behind alternative feature sets bears some resemblance to counterfactual explanations~\cite{guidotti2022counterfactual, stepin2021survey, verma2020counterfactual}, which uncover solutions for alternative predictions.
