...
Our experiments show that finding alternative feature sets with similar feature-set quality is possible indeed.
...
In our article, we base dissimilarity on the Dice coefficient, a simple and well-known set-similarity measure:
%
\begin{equation}
...
We mostly use the default hyperparameters of the models, apart from choosing information gain as split criterion and setting a random seed for reproducibility.
...
As we do not regularize or prune the trees, this observation is expected.
However, this will not invalidate our analysis how prediction performance develops over alternative feature sets, where we normalize feature-set quality.
To a lesser extent than prediction performance, the optimization objective $Q$ also shows overfitting for all feature-selection methods.
...
Regarding test-set prediction performance, simultaneous search does not even show lower variance than sequential search any more.
...
In particular, 10\% of feature sets from simultaneous search were feasible but potentially sub-optimal, but no feature sets for sequential search.
