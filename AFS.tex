...
\end{proposition}
%
Thus, the runtime resides in the parameterized complexity class~$\mathcal{XP}$ with the parameter~$k$ and remains polynomial under the assumption $k \ll n,~k \in O(1)$.
...
The complexity remains polynomial in~$n$ if $a$ and~$k$ are small and independent from~$n$.
The problem is in the parameterized complexity class $\mathcal{XP}$ again, now with the parameter~$a \cdot k$.
...
We also placed it in the parameterized complexity class $\mathcal{XP}$ (cf.~Proposition~\ref{prop:afs:complexity-exhaustive-simultaneuos}), having~$a$ and~$k$ as the parameters that drive the hardness of the problem. 
...
As a first step in that direction, we show membership in complexity class~$\mathcal{APX}$ under certain conditions (cf.~Section~\ref{sec:afs:appendix:univariate-search}), i.e., there are polynomial-time algorithms yielding constant-factor approximations.
Further, there might be efficient exact or approximate algorithms for problem instances satisfying additional assumptions.
Finally, while we placed alternative feature selection in class~$\mathcal{XP}$, one might prove membership or hardness for more specific parameterized complexity classes.
...
Since \emph{Greedy Replacement Search} ensures to include the $\lfloor (1 - \tau) \cdot k \rfloor$~best features in each alternative, we obtain the following proposition:
%
\begin{proposition}[Quality of \emph{Greedy Replacement Search}]
	Assume univariate feature qualities of $n$~features, $a \in \mathbb{N}_0$~alternatives, a dissimilarity threshold~$\tau$, desired feature-set size~$k$, and $k + a \cdot \lceil \tau \cdot k \rceil \leq n$.
	Under these conditions, \emph{Greedy Replacement Search} reaches at least a fraction of $\lfloor (1 - \tau) \rfloor$ of the objective values of (1) sequential search, (2) simultaneous search with sum-aggregation, and (3) simultaneous search with min-aggregation.
	\label{prop:afs:approximation-greedy-replacement}
\end{proposition}

In particular, \emph{Greedy Replacement Search} yields a constant-factor approximation of the three mentioned optimization problems.
Further, the heuristic has a polynomial runtime of $o(n \cdot \log n)$ since it sorts all~$n$ feature qualities first and then iterates over each feature at most once.
TODO: also consider array access on "indices" and $s_j$
Thus, alternative feature selection resides in the complexity class~$\mathcal{APX}$ under the given conditions.
...
In particular, by always selecting the $\lfloor (1 - \tau) \cdot k \rfloor$~best features, the heuristic misses out on feature sets only involving the next best features.
%
\begin{example}
...
\begin{algorithm}[tp]
	...
	$indices \leftarrow$ sort\_indices($q$, order=descending) \tcp*{Order by qualities} \label{al:afs:greedy-balancing:line:common-features-start}
	\For(\tcp*[f]{Initial selection for all alternatives}){$i \leftarrow 0$ \KwTo $a$}{
		$s^{(i)} \leftarrow \{0\}^n$ \;
	}
	$feature\_position \leftarrow 1$ \tcp*{Index of index of current feature}
	\While(\tcp*[f]{Select best features}){$feature\_position \leq \lfloor (1 - \tau) \cdot k \rfloor$}{
		$j \leftarrow indices[feature\_position]$ \tcp*{Index feature by quality}
...
\begin{proposition}[Quality of \emph{Greedy Balancing Search}]
	Assume univariate feature qualities of $n$~features, $a \in \mathbb{N}_0$~alternatives, a dissimilarity threshold~$\tau$, desired feature-set size~$k$, and $k + a \cdot \lceil \tau \cdot k \rceil \leq n$.
	Under these conditions, \emph{Greedy Balancing Search} reaches at least a fraction of $\lfloor (1 - \tau) \rfloor$ of the objective values of (1) sequential search, (2) simultaneous search with sum-aggregation, and (3) simultaneous search with min-aggregation.
...
\begin{algorithm}[tp]
	...
	\For(\tcp*[f]{Start with $k$ best features}){$p \leftarrow 1$ \KwTo $k$}{
...
$s^{(0)}$ contains these $k$ best features, i.e., $s^{(0)} = (1,1,1,1,0,0)$.
...
For further alternatives, the situation becomes more involved, because the alternatives do not contain all best $\lfloor (1 - \tau) \cdot k \rfloor$~features anymore.
Also, already for the first few alternatives, the runtime behavior differs from the other two heuristics.
In particular, \emph{Greedy Replacement Search} has the same runtime cost between subsequent alternatives since it directly creates valid alternative.
In contrast, \emph{Greedy Depth Search} iterates over all possible feature sets and the runtime between alternatives may vary.
In the worst case, \emph{Greedy Depth Search} finishes after $O(n^k)$ feature sets when it has exhausted further potential alternatives.
...
It may yield more alternative than the former two heuristics but has a more variable runtime behavior, as discussed before.
