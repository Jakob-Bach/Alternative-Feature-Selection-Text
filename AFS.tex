...
\textsc{Partition} asks if one can partition a set of elements with positive weights into two subsets with the same sum over their elements weights, while \textsc{3-Partition} demands a partitioning into subsets containing three elements whose weights all sum up to one particular pre-defined number.
...
In particular, it resembles the \textsc{Multi-Way Number Partitioning} problem, which is about partitioning a multiset of $n$~numbers into $a$~subsets such that the sums of all subsets are as equal as possible~\cite{korf2010objective}.
...
The most prominent problem formulation minimizes the maximum sum of all subsets~\cite{lawrinenko2018reduction, walter2017improved}.
Following~\cite{garey2003computers}, we use the term \textsc{Multiprocessor Scheduling} for this problem.
