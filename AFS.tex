...
\end{align}
%
In the remainder of this section, we formalize the notion of \emph{being alternative} with constraints.
In Section~\ref{sec:approaches}, we discuss different objective functions and their consequences on solving the optimization problem.
...
This simple constraint type allows for using a broad range of solvers, as we discuss in Section~\ref{sec:approaches}.
...
In Section~\ref{sec:problem:single}, we already showed that our notion of alternative feature sets results in 0-1 integer linear constraints.
...
For univariate filter feature selection, the objective function is linear as well.
...
\end{equation}
%
Here, $q$ typically is a bivariate dependency measure, e.g., mutual information or absolute value of Pearson correlation, to quantify the relationship between a feature and the prediction target.

\subsubsection{Post-Hoc Feature Importance}
...
\subsubsection{Multivariate Filter Feature Selection}
...
\begin{align}
	...
	\text{subject to:} &\quad s_{j_1} \cdot s_{j_2} \cdot q(X_{\cdot{}j_1}, X_{\cdot{}j_2}) < q(X_{\cdot{}j_1},y) \quad\forall (j_1,j_2) \in \{1, \dots, n\}^2, j_1 \neq j_2
	\label{eq:fcbf}
\end{align}
%
...
If one considers alternative feature selection as multi-objective problem, as mentioned in Section~\ref{sec:problem:structure}, there are no hard constraints any more.
...
\subsection{Embedding Alternatives}
...
- baseline: let solver enumerate or sample
