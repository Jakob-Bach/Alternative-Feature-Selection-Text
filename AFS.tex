...
\subsubsection{Relationship to NP-Hard Problems}
...
Thus, we partition the features into $a+1$ `actual' feature sets and potentially a $a+2$-th set containing all remaining features, which we call the \emph{dummy feature set}.
If the latter exists, we speak of a \emph{incomplete partitioning}, else a \emph{complete partitioning}.
Also, we assume univariate feature qualities as in Equation~\ref{eq:afs:univariate-filter} and Section~\ref{sec:afs:appendix:complete-optimization-problem}, i.e., a feature set's quality equals the sum of its constituent features' qualities.

Several number-partitioning problems are proven to be NP-complete~\cite{garey2003computers}, e.g., \textsc{Partition}~\cite{karp1972reducibility} and \textsc{3-Partition}~\cite{garey2003computers}.
...
These numbers roughly scale like $a^n / a!$~\cite{moser1958stirling}, which is exponential in~$n$.

Unlike \textsc{Partition} and \textsc{3-Partition}, we do not require alternative feature sets to have exactly the same quality.
For the summed-quality objective according to Equation~\ref{eq:afs:afs-simultaneous-sum-objective}, alternative feature selection with univariate feature qualities and~$\tau = 1$ is easy.
...
We only need to make sure that each feature set conforms to the user-specified cardinality.
Thus, we only need to iterate over the features once and thereby obtain a complexity of $O(n)$.
...
Due to the quality-based sorting, the complexity is~$O(n \cdot \log n)$, i.e., still polynomial.
Assuming $k \ll n$ and $a \ll n$, the complexity even becomes~$O((a+1) \cdot k \cdot n) \sim O(n)$ since we can iteratively determine the feature with maximum quality rather than sorting all qualities.
...
Different objectives formalizes the latter notion and can lead to different results~\cite{korf2010objective}.
One problem formulation, minimizing the maximum sum of all sets, is called \textsc{Multiprocessor Scheduling}.
This objective is equivalent to our objective if one multiplies all number with -1, turning the mini-max problem into the maxi-min formulation we use.
Since \textsc{Multiprocessor Scheduling} is NP-complete~\cite{garey2003computers}, our problem is NP-complete as well.
For a fixed number of partitions, \textsc{Multiprocessor Scheduling} allows for pseudo-polynomial algorithms.
These algorithms run in polynomial time if the input numbers are bounded in their size, e.g., small integers.
However, our feature-set qualities are real numbers in $[0,1]$ unless one scales and discretizes them.

Note that the previous result assumes a complete partitioning and feature sets of arbitrary sizes.
However, feature selection in general and our paper in particular typically work with a fixed feature-set size~$k$.
Depending on the number of alternatives, the partitioning then typically is incomplete.
