...
Ideally, the alternative feature sets should have similar quality.
However, depending on how different the alternatives should be, one might have to compromise on quality.
...
Users should be able to control the dissimilarity of alternatives and hence this quality trade-off.
...
Finally, this approach gives users control over the search for alternatives with two parameters, i.e., the number of alternatives and a dissimilarity threshold for feature sets being alternative. 
...
Simultaneously, the runtime is polynomial if the feature-set sizes and the number of alternatives are fixed.
Third, we discuss how to solve this optimization problem.
To that end, we describe how to integrate different categories of conventional feature-selection methods in the objective function.
...
Thus, these parameters allow users to exercise use-case-specific control on the trade-off between dissimilarity and quality of alternatives.
Also, depending on the number of features in the dataset, even no valid alternatives might exist if the parameter values are too strict.
Further, we observe that the prediction performance of feature sets may only weakly correlate with the quality assigned by feature-selection methods.
...
Section~\ref{sec:afs:fundamentals} introduces the notation and fundamentals of feature selection.
...
The appendix (cf.~Section~\ref{sec:afs:appendix}) contains technical details.
