...
\newtheorem{definition}[proposition]{Definition} % re-use counter of propositions
...
In our experiments, we observe that alternatives might indeed have high prediction quality, though several factors influence this outcome.
\end{abstract}
%
...
Ideally, the alternative feature set should have similar quality.
However, depending on how different the alternative feature set should be, one might have to compromise on quality.
E.g., if there are only a few highly predictive features and most of them are part of one feature set, other alternatives might have significantly lower prediction quality.
...
We focus on the feature-set quality of the alternatives relative to user parameters.
...
Experimentally, we could often find alternative feature sets with similar quality.
...
Further, let $y \in Y^m$ represent the prediction target with domain $Y$, e.g., $Y=\{0,1\}$ for binary classification or $Y=\mathbb{R}$ for regression.
...
With sum-aggregation, both these solutions are optimal, which might not satisfy the user's needs in some scenarios.
...
Given the monotonicity of the univariate objective with min-aggregation, adding feature~$f'$ to a feature set in the solution of~$I'$ does not have any benefit since~$f'$ would replace a feature with higher quality.
