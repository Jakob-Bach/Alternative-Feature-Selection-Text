...
These heuristics may be faster than exact optimization, at the expense of potentially lower feature-set quality.
...
The second search method is simultaneous, the other two are sequential.
...
Consequently, the bound also applies to minimum or sum of qualities over feature sets.
...
Given the polynomial runtime of this heuristic, we obtain membership in complexity class $\mathcal{APX}$~\cite{khanna1998syntactic}:
%
...
\end{proposition}
%
In case~$\tau = 1$, the heuristic yields the same objective values as sequential search and simultaneous search with sum-aggregation since it becomes identical to a procedure we outlined in our complexity analysis earlier (cf.~Proposition~\ref{prop:afs:complexity-partitioning-sum}).
The following example shows that the heuristic can be worse than exact search with sum-aggregation with as few as $a=2$ alternatives:
%
\begin{example}
...
For min-aggregation, $a=1$ alternative already suffices such that the heuristic may be worse than exact search:
%
\begin{example}
...
Simultaneous search with min-aggregation yields the selection $s^{(0)} = (1,1,0,0,1,0)$ and $s^{(1)} = (1,0,1,1,0,0)$, with quality of $\min~\{19,19\} = 19$.
\emph{Greedy Replacement Search} and sequential search all yield the selection $s^{(0)} = (1,1,1,0,0,0)$ and $s^{(1)} = (1,0,0,1,1,0)$, with a quality of $\min~\{24,14\} = 14$.
Simultaneous search with sum-aggregation may yield any of these two solutions or the selection $s^{(0)} = (1,1,0,1,0,0)$ and $s^{(1)} = (1,0,1,0,1,0)$ with the same summed quality.
\label{ex:afs:greedy-replacement:worse-than-min-agg}
\end{example}
%
The issue is that \emph{Greedy Replacement Search}, as a sequential search method, does not balance feature-set qualities.
...
Further, the algorithm assumes that all alternatives have the same size~$k$.
Finally, like exact sequential search, \emph{Greedy Replacement Search} does not balance the qualities of the alternatives.
...
\end{proposition}
%
\emph{Greedy Balancing Search} can be better than sequential search regarding the objective with min-aggregation, as Example~\ref{ex:afs:greedy-replacement:worse-than-min-agg} shows, where the heuristic would yield the same solution as simultaneous search with min-aggregation.
...
In contrast to the latter, the search does not restart from scratch for each alternative.

\emph{Greedy Depth Search} can yield more alternatives than \emph{Greedy Replacement Search} with its $O(n^2)$ cost or \emph{Greedy Balancing Search} with its $O(a \cdot n^2)$ cost.
...
For further alternatives, a quality analysis becomes more involved, because the alternatives do not contain all top $\lfloor (1 - \tau) \cdot k \rfloor$~features anymore.
Thus, we leave this analysis open for future work.
In fact, the quality of alternatives is not even monotonically decreasing anymore, as the following example shows:
%
\begin{example}
