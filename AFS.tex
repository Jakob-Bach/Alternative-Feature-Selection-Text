...
Appendix~\ref{sec:afs:appendix:univariate-complete-optimization-problem} specifies the complete optimization problem including the constraints for alternatives.
Appendix~\ref{sec:afs:appendix:univariate-pre-selection} describes how to potentially speed up optimization by leveraging monotonicity of the objective.
...
An exhaustive search over the entire search space is the arguably simplest though inefficient approach to find alternative feature sets.
...
\begin{proposition}[Complexity of exhaustive conventional feature selection]
	Exhaustive search for one feature set of size~$k$ from $n$~features has a time complexity of~$O(n^k)$ without the cost for evaluating the objective function.
	\label{prop:afs:complexity-exhaustive-conventional}
\end{proposition}
%
Evaluating the objective means computing the quality of each solution candidate, so we can determine the best feature set in the end.
...
This procedure entails a cost of~$O(a \cdot n)$ for one alternative and~$O(a^2 \cdot n)$ for the whole sequential procedure with~$a$ alternatives.
...
\end{proposition}
%
Thus, assuming $k \ll n,~k \in O(1)$, the runtime remains polynomial and resides in the parameterized complexity class~$\mathcal{XP}$ with~$k$ as parameter.
...
Including the cost for constraint checking, we arrive at the following proposition:
%
...
\end{proposition}
%
The scalability with~$n$ is clearly worse than for sequential search since the number of alternatives appears in the exponent now, except for a special case discussed in Appendix~\ref{sec:afs:appendix:complexity:exhaustive-simultaneous-special-case}.
...
The complexity still remains polynomial in~$n$ if $a$ and~$k$ are small and independent from~$n$.
For parameterized complexity, the problem is in $\mathcal{XP}$ again, now with the parameter~$a \cdot k$.
...
While assumptions like $a \cdot k \in O(1)$ ensure polynomial runtime for arbitrary feature-selection methods, the optimization problem without these assumptions can still be hard.
...
This scenario differs from the one for which we made for polynomial-runtime claims in Section~\ref{sec:afs:approach:complexity:exhaustive}.
...
The most prominent problem formulation, called \textsc{Multiprocessor Scheduling} in~\cite{garey2003computers}, minimizes the maximum subset sum~\cite{lawrinenko2018reduction, walter2017improved}.
Figuratively speaking, the goal is to assign tasks with different lengths to a fixed number of processors such that the maximum processor runtime is minimal.
...
The tasks become features, the task lengths become univariate feature qualities, and the processors become feature sets. 
...
The key reason for polynomial runtime is that sum-aggregation does not entail any requirements on balancing the feature sets' qualities.
Thus, $\tau=1$ allows for many solutions that all have the same objective value.
While at least one of these solutions also optimizes the objective with min-aggregation, most of them do not.
Hence, it is not a contradiction that optimizing with min-aggregation is harder.
...
We also placed it in the parameterized complexity class $\mathcal{XP}$ (cf.~Propositions~\ref{prop:afs:complexity-exhaustive-sequential} and~\ref{prop:afs:complexity-exhaustive-simultaneuos}), having~$a$ and~$k$ as the parameters that drive the hardness of the problem. 
...
Section~\ref{sec:afs:evaluation:summary} summarizes our key findings.
...
Even if training feature-set quality is similar, some alternatives might generalize better, i.e., maintain their quality on the test set, while other alternatives might loose more quality.
...
We observe that all search methods yield very similar distribution of feature-set quality.
This observation holds for all four white-box feature selection methods, while the heuristic \emph{Greedy Wrapper} even favors sequential search.
In contrast, for \emph{MI}, as visible in Figure~\ref{fig:afs:impact-search-mean-train-objective}, and \emph{Model Gain}, simultaneous search tends to develop a slight advantage with a growing number of alternatives.
...
While simultaneous search can yield better feature sets in some scenarios, sequential search can also be better in other scenarios.
...
A major reason for simultaneous search failing to consistently beat sequential search quality-wise is that search results can be suboptimal.
...
In particular, for up to five alternatives and $k=5$, all sequential searches for \emph{FCBF}, \emph{MI}, and \emph{Model Gain} finished within the timeout, i.e., yielded the optimal feature set or ascertained infeasibility, while \emph{mRMR} had about 10\% timeouts.
In contrast, for simultaneous search with sum-aggregation, all feature-selection methods experience timeouts:
Roughly 2\% of the searches for \emph{FCBF}, \emph{MI}, and \emph{Model Gain}, and 67\% of the searches for \emph{mRMR} found a feasible solution but could not proof optimality.
Such timeout-affected simultaneous solutions can be worse than optimal sequential solutions.
Also, a few simultaneous search runs for \emph{FCBF} indicate \emph{not solved}, i.e., the solver did not find a feasible solution without being able to proof infeasibility.
Min-aggregation instead of sum-aggregation in simultaneous search increases the number of timeouts for \emph{MI} and \emph{Model Gain} but decreases them for \emph{FCBF} and \emph{mRMR}.
Still, sequential search incurs less timeouts for all these four feature-selection methods.
...
For simultaneous search with $k=5$ and sum-aggregation, roughly 7\% of the white-box searches timed out for~$a=1$ but 20\% for~$a=3$ and 29\% for~$a=5$.
Despite we grant simultaneous searches proportionally more time for multiple alternatives, the observed increase in timeouts suggests that runtime increases super-proportionally, as we analyze next.
...
\begin{table}[t]
	...
	\end{tabular}
	\caption{
		Mean optimization time by feature-selection method and number of alternatives.
...
Further, \emph{MI}, and \emph{Model Gain} experiences a dramatic increase in optimization time with the number of alternatives~$a$ in simultaneous search, as Table~\ref{tab:afs:impact-num-alternatives-fs-method-optimization-time} displays.
In contrast, the runtime increase is considerably less for sequential search, which shows an approximately linear trend with the number of alternatives there.
...
However, the observed trend is rather noisy, and some higher-dimensional datasets even show lower average runtimes than lower-dimensional datasets.
This results indicates that several other factors than~$n$ influence runtime.
Besides factors related to the datasets and experimental design, the heuristics used by the solver may also play a role, e.g., work better for some problem instances than for others.
...
Figure~\ref{fig:afs:impact-num-alternatives-quality-max} shows that multiple alternatives may have similar quality, as the median training-set objective value remains relatively stable over the alternatives and is above~0.8 even for the tenth alternative.
...
For further alternatives, the value of this metric is stable.
Additionally, the initial decrease is less prominent than on the training set.
...
However, the more alternatives are desired, the likelier an infeasible optimization problem is.
...
Increasing the feature-set size~$k$ or having lower dataset dimensionality~$n$ naturally causes more infeasible solutions, as less features become available for alternatives.
...
In these figures, the downward trend of feature-set quality over the alternatives becomes slightly more prominent, particularly for a high number of alternatives.
...
While we discussed \emph{MI} before, the decrease of objective value over the number of alternatives occurs for all feature-selection methods in our experiments, as Figure~\ref{fig:afs:impact-num-alternatives-fs-method-train-objective-max} displays.
...
We use results from $k=10$ instead of $k=5$ here to show more distinct values of $\tau$.
...
This is expected, since a higher~$\tau$ constrains the feature selection more.
...
Similar to our analysis for the number of alternatives (cf.~Section~\ref{sec:afs:evaluation:num-alternatives}), one needs to consider that setting~$\tau$ to certain values can make the optimization problem infeasible.
...
In contrast, the remaining three feature-selection methods exhibit little influence of~$\tau$ on the quality unless one also accounts for infeasible feature sets (cf.~Figure~\ref{fig:afs:impact-tau-fs-method-train-objective-max-fillna}).
For \emph{Greedy Wrapper}, this outcome may be explained by the heuristic, inexact search procedure.
For \emph{FCBF}, the fraction of infeasible solutions is generally much higher than for \emph{MI}, which influences the quality distribution of found feature sets.
For \emph{mRMR}, the low influence of~$\tau$ matches the low influence of the number of alternatives.
For this feature-selection method, alternatives seem to generally vary little in their objective value.
Finally, the test-set prediction performance does not vary considerably over $\tau$ for any of the feature-selection methods, as Figure~\ref{fig:afs:impact-tau-fs-method-decision-tree-test-mcc-max} displays.
...
Different notions of feature-set quality exhibited trends in our experiments, so one should choose a notion of feature-set quality carefully.
In particular, the objective function of feature-selection methods might disagree with actual prediction performance of the corresponding feature sets.
...
Finally, selecting $k=10$ instead of $k=5$ features only yielded a small improvement in prediction performance for all feature-selection methods, so one might stick to smaller feature-set sizes if such a setting benefits interpretability for users.
...
Also, sequential search allows users to stop search after each alternative instead of requiring the number of alternatives to be specified beforehand.
...
In particular, the frequency of infeasible solutions increased with~$a$ due to the presence of more constraints.
...
A higher dissimilarity threshold caused a stronger decrease of feature-set quality in terms of objective value for the feature-selection methods \emph{MI} and \emph{Model Gain}.
...
\paragraph{Computational complexity} Appendix~\ref{sec:afs:appendix:complexity:future-work} discuses how one could extend our complexity analysis of alternative feature selection (cf.~Section~\ref{sec:afs:approach:complexity}).
...
While we could uncover several general trends, the existence and the quality of alternatives clearly depends on the dataset (cf.~Appendix~\ref{sec:afs:appendix:evaluation:datasets}).
...
Section~\ref{sec:afs:appendix:multivariate-filter-objectives} discusses further objective functions for multivariate filter feature selection (cf.~Section~\ref{sec:afs:approach:objectives:white-box}).
Section~\ref{sec:afs:appendix:univariate-complete-optimization-problem} provides complete definitions of the alternative-feature-selection problem (cf.~Section~\ref{sec:afs:approach}).
Section~\ref{sec:afs:appendix:univariate-pre-selection} proposes how to speed up optimization for univariate feature selection (cf.~Equation~\ref{eq:afs:univariate-filter}).
Section~\ref{sec:afs:appendix:complexity} amends the complexity analysis (cf.~Section\ref{sec:afs:approach:complexity}).
...
%
\begin{example}
Consider univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter}) with $n=6$~features, feature qualities~$q = (9,8,7,3,2,1)$, feature-set size~$k=3$, number of alternatives~$a=2$, and dissimilarity threshold~$\tau = 0.5$, which permits an overlap of one feature between sets here.
...
%
\begin{example}
Consider univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter}) with $n=6$~features, feature qualities~$q = (11,10,6,5,4,1)$, feature-set size~$k=3$, number of alternatives~$a=1$, and dissimilarity threshold~$\tau = 0.5$, which permits an overlap of one feature between sets here.
...
First, work on number partitioning also use other objectives for balancing~\cite{korf2010objective, lawrinenko2017identical} (cf.~Section~\ref{sec:afs:appendix:complexity:related-work}).
...
In this section, we describe how to potentially speed up the optimization for the objective with univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter}).
This objective is monotonic in the individual features' qualities~$q(X_{\cdot{}j},y)$ and the feature-selection decisions~$s_j$.
In particular, the objective cannot decrease if one selects more features or, given a fixed feature-set size~$k$, replaces one feature with another one of higher quality.
...
The latter has polynomial runtime under the assumption of small, constant $a$ and $k$, i.e., $a \cdot k \in O(1)$ (cf.~Section~\ref{sec:afs:approach:complexity:exhaustive}).
With pre-selection, the pure search cost would even become independent from~$n$, i.e., $O(1)$, under that assumption.
However, one needs to determine the highest feature qualities first, e.g., by sorting all qualities in~$O(n \cdot \log n)$ or iteratively determining the maximum quality in~$O((a+1) \cdot k \cdot n)$.
...
There, each feature set is alternative to each other unless both sets are identical.
Thus, each set of $a + 1$ distinct feature sets constitutes a valid solution and further constraint checking is unnecessary.
Hence, instead of iterating over sets of feature sets, once can iterate over individual feature sets and maintain a buffer containing the $a + 1$ feature sets with highest quality.
...
I.e., unlike in Proposition~\ref{prop:afs:complexity-exhaustive-simultaneuos}, the number of alternatives~$a$ is not part of the exponent anymore and the cost corresponds to the search for one feature set times the cost for updating the buffer.
For large $a$, one can implement the buffer as a heap and thereby reduce the linear factor regarding~$a$ to a logarithmic one.
...
As discussed in Section~\ref{sec:afs:approach:objectives:white-box}, the univariate objective and several other feature-selection methods allow to phrase alternative feature selection as a 0-1 integer linear program.
...
However, since alternative feature selection only uses particular constraint types instead of expressing arbitrary integer linear problems, it could still be easier.
...
In particular, different objectives formalize notion of balanced subset sums and can lead to different optimal solutions~\cite{korf2010objective, lawrinenko2017identical}.
...
Further, besides approximations, the problem also has polynomial-time exact algorithms if certain parameters of the problem are fixed, e.g., the number of distinct numbers to be partitioned or the largest number~\cite{mnich2018parameterized}.
...
While the previous approaches considered feature sets of arbitrary sizes, there are number-partitioning problems with constrained~$k$ as well, e.g., called \textsc{Balanced Number Partitioning} or \textsc{K-Partitioning}.
The problems formulations differ in their objective and cardinality constraints, e.g., if equalities or inequalities are used.

For the minimax objective, \cite{babel1998thek, michiels2012computer, zhang2011heuristic} propose heuristic algorithms, some of them with approximation guarantees.
\cite{babel1998thek} also provides a bound of the objective value relative to the unconstrained case.
Further, there is a PTAS for each fixed partition size~$k$~\cite{michiels2012computer}.
...
Again, this problem is strongly $\mathcal{NP}$-hard~\cite{he2003kappa, lawrinenko2018reduction}, but \cite{chen20023partitioning, he2003kappa, lawrinenko2018reduction} propose approximation algorithms, partly with quality guarantees.
...
\textsc{3-Partition}~\cite{garey2003computers} demands a partitioning into three-element subsets whose positive integer weights all sum up to one pre-defined number.
...
However, our problem is a special case, where the feature qualities act as profits, the feature-set sizes are capacities, and each feature has a weight of one.
...
However, if the qualities of the resulting alternatives are not equal, $f'$ might still be chosen in a feature set that does not have the minimum quality of all feature sets, since only the latter determines the overall objective value.
In that case, we simply replace $f'$ with the remaining feature that was not selected instead; the objective value remains the same and the solution becomes valid for~$I$.
...
Since checking a solution for incomplete partitioning only needs polynomial time, we obtain membership in $\mathcal{NP}$ and thereby $\mathcal{NP}$-completeness.
\end{proof}
...
\end{proof}
%
Note that adding the proposed~$f'$ with a high quality~$q'$ enlarges the size of the problem instance.
...
In particular, the encoding of a problem instance involves $n$~feature qualities and the values of $a$, $k$, and $\tau$.
...
The maximal number of features to be added is polynomial in~$k$ and thereby also polynomial in~$n$.
...
Again, the assignment of features to sets does not matter for the objective value.
...
In this section, we outline future work on alternative feature selection from the complexity-theory perspective, building on the Sections~\ref{sec:afs:approach:complexity} and~\ref{sec:afs:conclusion:future-work}.
...
In the former case, there might be  at least pseudo-polynomial approaches or (F)PTAS.
Further, there might be efficient algorithms for problem instances satisfying additional requirements.
Finally, while we placed alternative feature selection in class~$\mathcal{XP}$, one might proof membership or hardness for more specific parameterized complexity classes.
...
Another interesting question is how many alternatives, regardless of their quality, exist for a given $n$, $k$, and $\tau$.
...
For an objective consisting of univariate feature qualities (cf.~Equation~\ref{eq:afs:univariate-filter} and Section~\ref{sec:afs:appendix:univariate-complete-optimization-problem}), one can easily find a limited number of optimal alternative feature sets without a solver.
...
At most, one could start with the original feature set from a sophisticated feature-selection method and then continue with greedy replacement based on univariate feature qualities.
...
In particular, we cover three experimental dimensions not stemming from to the search for alternatives:
datasets (cf.~Section~\ref{sec:afs:appendix:evaluation:datasets}), evaluation metrics (cf.~Section~\ref{sec:afs:appendix:evaluation:metrics}), and feature-selection methods (cf.~Section~\ref{sec:afs:appendix:evaluation:feature-selection}).
...
Figure~\ref{fig:afs:impact-dataset-k-quality} depicts the relationship between datasets and quality of the original, i.e., unconstrained, feature set in sequential search.
To account for the varying dataset dimensionality, we put the ratio between feature-set size~$k$ and dimensionality~$n$ on the x-axis, which is a measure of relative feature-set sizes.
...
This variation supports our normalization of feature-set of quality when analyzing alternatives in Sections~\ref{sec:afs:evaluation:num-alternatives} and~\ref{sec:afs:evaluation:tau}.
...
Further, both model types exhibit overfitting, i.e., there is a gap between training-set and test-set prediction performance.
In particular, over all experimental settings, both model types have a mean training-set MCC of 0.85 (median: 1.0).
In contrast, decision trees have a mean MCC of 0.48 (median: 0.54) on the test set, while random forests have a slightly higher mean MCC of 0.52 (median: 0.63).
...
However, this phenomenon does not invalidate our analysis of how prediction performance develops over alternatives.
...
Thus, Section~\ref{sec:afs:evaluation} considers training set and test set for the objective value, but only the test-set for prediction performance.
...
This two-step procedure accounts for the different objectives of feature-selection methods and for the normalization of quality per dataset and cross-validation fold in some objectives (cf.~Section~\ref{sec:afs:experimental-design:approaches:feature-selection}).
...
Among the five feature-selection methods, \emph{Greedy Wrapper} has the highest correlation between training-set objective value and test-set MCC, with a value of 0.43.
...
As the five feature-selection methods employ different objective functions~$Q$, it does not make sense to compare absolute objective values between them.
However, we can analyze prediction performance of the obtained feature sets.
...
The median test-set MCC of decision trees is 0.59 for \emph{Model Gain}, 0.57 for \emph{FCBF}, and 0.54 for \emph{MI}, 0.53 for \emph{mRMR}, and 0.46 for \emph{Greedy Wrapper}.
...
Further, \emph{FCBF}'s results may be taken with a grain of salt:
Over all experimental settings, 89\% of feature sets for \emph{FCBF} were infeasible, i.e., no feature set satisfied the constraints.
...
As one can expect, larger feature sets usually exhibit higher feature-set quality than smaller feature sets in our experiments.
However, the increase in quality with $k$ is not proportional, and there might even be a decrease.
As Figure~\ref{fig:afs:impact-fs-method-k-metric-diff} shows for the original feature sets of sequential search, \emph{MI}, \emph{FCBF}, and \emph{Model Gain} exhibit an increase of the training-set objective value~$Q_\text{train}$ from~$k=5$ to~$k=10$, i.e., the difference depicted in Figure~\ref{fig:afs:impact-fs-method-k-metric-diff} is positive.
As these objectives are monotonic in the set of selected features, a decrease in the training-set objective value is not possible.
In contrast, the heuristic \emph{Greedy Wrapper} does not necessarily benefit from more features.
The latter insight applies to \emph{mRMR} as well since it normalizes its objective with the number of selected features and penalizes feature redundancy.
