...
\begin{algorithm}[t]
	...
	$feature\textunderscore{}position \leftarrow 1$ \tcp*{Index of index of current feature}
	\While{$feature\textunderscore{}position \leq \lfloor (1 - \tau) \cdot k \rfloor$}{
		$j \leftarrow indices[feature\textunderscore{}position]$ \tcp*{Index feature by quality}
		$s_j \leftarrow 1$ \;
		$feature\textunderscore{}position \leftarrow feature\textunderscore{}position + 1$\;
	...
	\While{$i \leq a$ \textbf{and} $i \leq \frac{n - k}{\lceil \tau \cdot k \rceil}$}{ \label{al:afs:greedy-replacement:line:stop}
		...
		\For(\tcp*[f]{Select remaining $\lceil \tau \cdot k \rceil$ features}){$\_ \leftarrow 1$ \KwTo $\lceil \tau \cdot k \rceil$}{
			$j \leftarrow indices[feature\textunderscore{}position]$\;
			$s^{(i)}_j \leftarrow 1$\;
			$feature\textunderscore{}position \leftarrow feature\textunderscore{}position + 1$\;
		}
		$i \leftarrow i + 1$\;
...
In each alternative, we select the $\lceil \tau \cdot k \rceil$~highest-quality features that are not part of any prior alternative.
...
The resulting search method is simultaneous rather than sequential.
...
\begin{algorithm}[tp]
	...
	$feature\textunderscore{}position \leftarrow 1$ \tcp*{Index of index of current feature}
	\While(\tcp*[f]{Select best features}){$feature\textunderscore{}position \leq \lfloor (1 - \tau) \cdot k \rfloor$}{
		$j \leftarrow indices[feature\textunderscore{}position]$ \tcp*{Index feature by quality}
	...
	\While(\tcp*[f]{Fill all positions}){$feature\textunderscore{}position \leq \lceil \tau \cdot k \rceil \cdot a + k$}{ \label{al:afs:greedy-balancing:line:stop}
