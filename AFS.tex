...
\begin{proposition}[Complexity of exhaustive simultaneous search]
	Exhaustive simultaneous search for $a + 1$~alternative feature sets of size~$k$ from $n$~features has a time complexity of~$O(a^2 \cdot n^{k \cdot (a+1) + 1})$ without the cost of evaluating the objective function.
...
However, the two objectives are prediction performance and feature-set size, while diversity only influences the selection step under particular circumstances.
...
Further, there might be efficient algorithms for problem instances satisfying additional assumptions.
