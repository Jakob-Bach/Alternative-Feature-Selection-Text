...
Since feature-set quality varies between datasets and cross-validation folds, as discussed in Appendix~\ref{sec:afs:appendix:evaluation:datasets}, we additionally normalize feature-set quality here.
...
\subsection{Dissimilarity Threshold \texorpdfstring{$\tau$}{}} % \texorpdfstring prevents warning "Token not allowed in a PDF string"
...
Similar to our previous analysis for the number of alternatives in Section~\ref{sec:afs:evaluation:num-alternatives}, one also needs to consider that setting~$\tau$ to certain values can make the optimization problem infeasible.
