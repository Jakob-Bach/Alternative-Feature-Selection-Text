...
Given two feature sets $F'$, $F''$, the Jaccard distance is defined as
%
\begin{equation}
...
\end{equation}
%
If $|F'| = |F''|$, this measure is also identical to several further overlap measures~\cite{egghe2009new}.
...
In fact, one can replace the product $s'_j \cdot s''_j$ with linear constraints by introducing an auxiliary variable $t_j$~\cite{mosek2022modeling}:
%
\begin{equation}
	\begin{aligned}
		t_j \leq& s'_j \\
...
\end{equation}
%
If there is an existing feature set, i.e., we either know $s'$ or $s''$, Equation~\ref{eq:feature-set-size} already is linear without Equation~\ref{eq:product-linear}.
...
The objective function remains the same as in the base case, i.e., we optimize the quality of the new feature set~$F''$.
